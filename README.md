...
## 1. Client Apps
**Mobile and Laptop:** These are the client devices from which users interact with the application. They send HTTP requests to the API Gateway.

## 2. API Gateway (Spring Cloud Gateway)
**Purpose:** Acts as a single entry point for all client requests, handling routing, client-side load balancing, authentication, and other cross-cutting concerns.

**Flow:**
- Receives requests from client devices.
- Routes the requests to the appropriate microservice (Item, Order, Cart, Customer) based on the request path.

## 3. Microservices
Each microservice (Item, Order, Cart, Customer) is a separate component responsible for specific functionality within the shopping application. They communicate with each other and the API Gateway to perform various operations.

### Item Service
- **Database:** Manages items' data including details like item name, description, price, stock, etc.
- **Responsibilities:** Handle item-related operations like retrieving item details, updating stock, etc.

### Order Service
- **Database:** Manages order data including order details, status, and associated customer information.
- **Responsibilities:** Handle order-related operations such as creating new orders, updating order status, etc.
- **Kafka Integration:** Sends messages to the Kafka message queue for asynchronous communication with the Payment Service.

### Cart Service
- **Database:** Manages cart data including items added to the cart by the customer.
- **Responsibilities:** Handle cart-related operations like adding items to the cart, updating cart items, etc.

### Customer Service
- **Database:** Manages customer data including customer profiles, addresses, and authentication details.
- **Responsibilities:** Handle customer-related operations such as user registration, login, profile updates, etc.

## 4. Kafka (Message Queue)
**Purpose:** Facilitates asynchronous communication between the Order Service and the Payment Service.

**Flow:**
- Order Service sends payment-related messages to the Kafka queue.
- Payment Service listens to the Kafka queue and processes the payment messages.

## 5. Payment Service
**Responsibilities:** Handle payment processing operations like payment validation, transaction processing, and updating order payment status based on Kafka messages.
...
**Flow:**
- Microservices (Item, Order, Cart, Customer) register themselves with the Eureka Server.
- The API Gateway uses Eureka to discover the instances of these services for load balancing and routing.

### Config Server
**Purpose:** Centralizes configuration management for all microservices.

**Flow:**
- Microservices pull their configuration properties from the Config Server, which in turn retrieves the configurations from a GitHub repository.
